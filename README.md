...
*   **rF2 Shared Memory Plugin**: Download from [the plugin repository](https://github.com/dallasformula/rF2SharedMemoryMapPlugin).
    *   Copy `rFactor2SharedMemoryMapPlugin64.dll` to `Le Mans Ultimate/Plugins/`.
    *   Enable the plugin in the game settings or `CustomPluginVariables.JSON` if required (usually enabled by default).
...
3.  **Steering Axis**: Bind this to the **vJoy Device (Axis X)**. 
    *   *Tip:* You might need to move the vJoy slider in the vJoy Feeder demo app to detect it, or manually assign it if the game allows.
4.  **Force Feedback**:
...
3.  **Master Gain**: Set to 1.0 initially.
